...
# 2. Função de Envio com Regra de Escalamento
def enviar_alerta(destinatario, nome, id_acao, acao, prazo_dt, dias):
    # Escalamento de alçada
    alçada_text = ""
    if dias >= 15:
        alçada_text = "<p style='color: #991b1b; font-weight: bold; background-color: #fee2e2; padding: 10px; border-radius: 5px;'>🚨 ALERTA CRÍTICO DE DIRETORIA: Ação atrasada há mais de 15 dias. Este apontamento foi reencaminhado para o Comitê Executivo.</p>"
    elif dias >= 7:
        alçada_text = "<p style='color: #854d0e; font-weight: bold; background-color: #fef9c3; padding: 10px; border-radius: 5px;'>⚠️ ESCALAMENTO DE GESTÃO: Ação atrasada há mais de 7 dias. Notificação copiada para a liderança direta.</p>"
